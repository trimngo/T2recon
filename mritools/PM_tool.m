...
%   Right click drops point.
%   Shift-click deletes point
%   Point information stored as appdata of each h_axes,  labelled 'Points'
...
handlesPM.Points = cell(size(h_all_axes));
[handlesPM.Points{:}] = deal(EmptyPointStruct);
...
for ax_index = 1:length(handlesPM.Axes)
    ...
    for pt_idx = 1:size(PointStruct,2)
        if ~isempty(PointStruct(pt_idx).name)
            delete(PointStruct(pt_idx).h);
        end;
    end;
end;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
function  old_NextPlots = Restore_Points;
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for ax_idx = 1:length(handlesPM.Axes)
    h_axes = handlesPM.Axes(ax_idx);
    old_NextPlots{ax_idx} = get(h_axes, 'Nextplot');
    set(h_axes, 'Nextplot', 'add');
    if isappdata(h_axes, 'Points')
        PointStruct = getappdata(h_axes, 'Points');
        for pt_idx = 1:size(PointStruct,2)
            if ~isempty(PointStruct(pt_idx).name)
                PointStruct(pt_idx).h = Draw_Point(h_axes, ...
                    PointStruct(pt_idx).x, PointStruct(pt_idx).y, PointStruct(pt_idx).c);
            end;
        end;    
        handlesPM.Points{ax_idx} = PointStruct;
    end;
end;
guidata(fig2, handlesPM);
